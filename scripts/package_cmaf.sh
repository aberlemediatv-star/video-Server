#!/usr/bin/env bash
# Package fragmented MP4 renditions into CMAF-style HLS + DASH using Shaka Packager (Docker).
# Usage: ./scripts/package_cmaf.sh <renditions_dir> <package_out_dir> [segment_seconds]
# Env: SKIP_DOCKER_PULL=1 — kein docker pull; SHAKA_PACKAGER_IMAGE — anderes Image.
set -euo pipefail
...
mkdir -p "${PKG_ABS}/video_1080" "${PKG_ABS}/video_720" "${PKG_ABS}/video_480" "${PKG_ABS}/audio"
...
if run_packager "${streams[@]}" "${opts[@]}"; then
  ...
  echo "Shaka Packager schlug fehl (beide Aufrufvarianten)." >&2
  exit 1
fi

echo "Packaged to: $PKG_ABS (manifest.mpd, master.m3u8)"
